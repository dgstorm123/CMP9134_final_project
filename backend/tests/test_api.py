...
def test_register_new_user(client):
    r = client.post("/api/auth/register", json={
        "username": "newuser", "password": "pass1234"   # ← FIX #2: no "role" field
    })
...
def test_register_short_password(client):
    r = client.post("/api/auth/register", json={
        "username": "validname", "password": "short"    # < 8 chars
    })
    assert r.status_code == 422
...
def test_login_wrong_password(client):
    ...
    r = client.post("/api/auth/login", json={
        "username": "wrongpw", "password": "wrongwrong"
    })
...
def test_mission_stats_recon(client):
    r = client.post("/api/mission_stats", json={
        "type": 1, "dist": 50, "batt": 10
    })
    assert r.status_code == 200
