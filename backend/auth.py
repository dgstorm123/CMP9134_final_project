...
def register(body: RegisterRequest, db: Session = Depends(get_db)):
    ...
    )  # check ava of user -> exited -> code 409
    if existing:
        raise HTTPException(
    ...
    user = User(
        ...
        password_hash=hash_password(
            body.password
        ),  # using hash fuc brycpt never store as plaintext
...
def login(body: LoginRequest, db: Session = Depends(get_db)):
    ...
    ):  # Verify password with bcrypt -> if not fit -> flag error
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    )  # if right -> create JWT token -> give back to client
...
def seed_default_users(db: Session) -> None:
    ...
    ):  # if db emty create new account admin/admin123! with role Commander -> so still have people can control robot
        admin = User(
            username="admin",
